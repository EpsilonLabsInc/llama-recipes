...
def get_custom_dataset(dataset_config, tokenizer, split):
    ...
    for data in dataset:
        if data["parent_id"]:
            nodes[data["parent_id"]] = nodes.get(data["parent_id"], []) + [
                data["message_id"]
            ]
        else:
    ...
    dataset = dataset.map(
        lambda x: {"thread": get_threads_from_root(x["message_id"])},
        remove_columns=list(dataset.features),
    )
    dataset = dataset.map(
        lambda x: {"thread": [i for row in x["thread"] for i in row]}, batched=True
    )
    ...
    dataset = dataset.map(
        lambda x: to_dialog(x["thread"]), remove_columns=list(dataset.features)
    )
    dataset = dataset.map(
